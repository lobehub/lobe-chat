...
- For a small subset needed before a larger response arrives, reuse
  ...
  demonstrated need.
- Synchronous local restoration may run during pre-paint initialization, as in an
  `src/store/project/store.ts`'s `useLayoutEffect` hydration path. This is not permission to fetch APIs in
  effects. IndexedDB hydration is asynchronous: account for its readiness instead
...
screenshot. Assert no default-to-custom
flash, collapsing sections, cross-scope data, or indefinite loading on failure.
