...
// Mock i18next's t function for consistent duplicateSymbol
vi.mock('i18next', () => ({
  t: vi.fn((key, options) => {
    if (key === 'duplicateSymbol' && options?.ns === 'common') {
      return 'copy'; // Default symbol for tests
    }
    return key;
  }),
}));
...
describe('findNextAvailableTitle', () => {
  ...
  it('should handle complex cases with gaps and existing numbered titles', () => {
    ...
    const nextTitle2 = findNextAvailableTitle(baseTitle2, titleSet);
  ...
  it('should handle symbols that might be part of the base name if not careful with regex', () => {
    ...
    const nextTitle2 = findNextAvailableTitle(baseTitle2, titleSet2);
