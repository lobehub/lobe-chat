...
describe('genMessages', () => {
  ...
  it('should add systemRole message at the beginning', () => {
    ...
    const result = genChatMessages({ messages, systemRole });
  ...
  it('should add user message at the end', () => {
    ...
    const messages: ChatMessage[] = [{ content: 'Welcome', role: 'system' }];
    const result = genChatMessages({ message, messages });
    expect(result.at(-1).content).toBe(message);
