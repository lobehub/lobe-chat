...
const buildCloudMcpManifest = (params: {
  ...
  // If tools are in MCP format, need to convert to LobeChat api format
  ...
  installMCPPlugin: async (identifier, options = {}) => {
    ...
    try {
      ...
      if (resume) {
        ...
        // Find stdio type deployment options
        ...
        // 🌐 Check if cloudEndPoint exists: web platform + stdio type + haveCloudEndpoint exists
        ...
        if (hasCloudEndpoint) {
          ...
          // Step 4: Check if configuration is required
          if (result.needsConfig) {
            // Configuration required, pause installation process
            updateMCPInstallProgress(identifier, {
              checkResult: result,
            ...
            updateInstallLoadingState(identifier, undefined);
            return false; // Return false to indicate configuration is required
          }
      ...
      // set version
      if (manifest) {
        // set Version - use semver to compare version numbers and take the higher one
        const dataVersion = data?.version;
        const manifestVersion = manifest.version;

        if (dataVersion && manifestVersion) {
          // If both versions exist, compare and take the higher one
