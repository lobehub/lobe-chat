...
describe('klavisStore actions', () => {
  ...
  describe('loadUserKlavisServers', () => {
    it('should load servers from database', async () => {
      const { result } = renderHook(() => useToolStore());
      ...
      const mockPlugins = [
        {
          ...
          manifest: {
            api: [{ name: 'sendEmail', description: 'Send email', parameters: {} }],
          },
        },
      ];
      vi.mocked(lambdaClient.klavis.getKlavisPlugins.query).mockResolvedValue(mockPlugins as any);

      await act(async () => {
        await result.current.loadUserKlavisServers();
      });

      expect(result.current.servers).toHaveLength(1);
      expect(result.current.servers[0].identifier).toBe('gmail');
      expect(result.current.servers[0].serverName).toBe('Gmail');
    ...
    it('should handle load error gracefully', async () => {
      ...
      vi.mocked(lambdaClient.klavis.getKlavisPlugins.query).mockRejectedValue(
