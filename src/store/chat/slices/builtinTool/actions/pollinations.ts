...
import { pollinationsImageGenerationService } from '@/services/pollinationsImageGeneration';
...
export interface ChatPollinationsAction {
...
export const pollinationsSlice: StateCreator<
  ChatStore,
  [['zustand/devtools', never]],
  [],
  ChatPollinationsAction
> = (set, get) => ({
  generateImageFromPrompts: async (items, messageId) => {
    ...
    await pMap(items, async (params, index) => {
      ...
      let url = '';
      try {
        url = await pollinationsImageGenerationService.generateImage(params);
      } catch (e) {
