...
export const currentTopics = (s: SessionStore): ChatTopic[] => {
  ...
  // 按时间倒序
  const favTopics = topics.filter((t) => t.favorite).sort((a, b) => b.createAt - a.createAt);
  const defaultTopics = topics.filter((t) => !t.favorite).sort((a, b) => b.createAt - a.createAt);
