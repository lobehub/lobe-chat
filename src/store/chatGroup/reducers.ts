...
export const chatGroupReducers = {
  ...
  loadGroups: (state: ChatGroupState, { payload }: { payload: ChatGroupItem[] }) =>
    produce(state, (draft: ChatGroupState) => {
      ...
      draft.groupMap = payload.reduce((map: Record<string, ChatGroupItem>, group: ChatGroupItem) => {
        map[group.id] = group;
        return map;
      }, {});
  ...
  updateGroup: (
    ...
    produce(state, (draft: ChatGroupState) => {
      if (draft.groupMap[payload.id]) {
        ...
        const groupIndex = draft.groups.findIndex((group: ChatGroupItem) => group.id === payload.id);
        if (groupIndex !== -1) {
