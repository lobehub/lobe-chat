...
const SupervisorMessage = memo<SupervisorMessageProps>(({ id }) => {
  ...
  const item = useConversationStore(
    dataSelectors.getDisplayMessageById(id),
    isEqual,
  ) as UIChatMessage;
  ...
  // Keep using ChatStore for triggerSupervisorDecision as it's an action not yet migrated
