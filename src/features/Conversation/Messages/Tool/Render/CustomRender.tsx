...
const CustomRender = memo<CustomRenderProps>(
  ({
    ...
    // Use ConversationStore for state selectors
    const loading = useConversationStore(messageStateSelectors.isPluginApiInvoking(id));
    const [isEditing, setIsEditing] = useState(false);
    const { message } = App.useApp();
    // Get actions from ConversationStore
    const [updatePluginArguments, reInvokeToolMessage] = useConversationStore((s) => [
