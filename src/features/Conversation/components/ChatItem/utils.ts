...
export const processWithArtifact = (input: string = '') => {
  ...
  const thinkMatch = THINKING_TAG_REGEX.exec(input);

  // If the input contains the `think` tag, replace all line breaks with an empty string
  if (thinkMatch)
    output = input.replace(ARTIFACT_THINKING_TAG_REGEX, (match) =>
      match.replaceAll(/\r?\n|\r/g, ''),
    );
  const lobeThinkMatch = ARTIFACT_THINKING_TAG_REGEX.exec(input);
