...
export const createParagraphRehypePlugin = (tag: string) => () => (tree: any) => {
  visit(tree, (node, index, parent) => {
    if (node.type === 'element' && node.tagName === 'p' && node.children.length > 0) {
      const firstChild = node.children[0];
      if (firstChild.type === 'raw' && firstChild.value.startsWith(`<${tag}`)) {
        // 提取 lobeArtifact 的属性
    ...
    // }
    else if (node.type === 'raw' && node.value.startsWith(`<${tag}`)) {
      // 创建新的 lobeArtifact 节点
      const newNode = {
        children: [],
        properties: {},
...
export default createParagraphRehypePlugin;
