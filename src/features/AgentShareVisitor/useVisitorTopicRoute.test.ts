...
const setup = (initialEntry = '/a/my-bot/chat/tpc_1') => {
  let router: ReturnType<typeof createMemoryRouter>;
  const wrapper = ({ children }: PropsWithChildren) => {
    router = createMemoryRouter([{ element: children, path: '/a/:slugOrId/chat/:topicId?' }], {
...
describe('useVisitorTopicRoute', () => {
  it('updates the URL for selection and restores topics through back/forward', async () => {
    ...
    expect(getRouter().state.location.pathname).toBe('/a/my-bot/chat/tpc_2');
    ...
    expect(getRouter().state.location.pathname).toBe('/a/my-bot/chat');
  ...
  it('replaces the blank conversation URL once a topic is created', async () => {
    ...
    expect(getRouter().state.location.pathname).toBe('/a/my-bot/chat/tpc_created');
    await act(() => getRouter().navigate(-1));
    expect(result.current.topicId).toBe('tpc_1');
  });

  it('does not push a duplicate entry when the destination is already current', async () => {
    const { result, getRouter } = setup('/a/my-bot/chat');
  ...
  it('does not navigate back when a send finishes after selecting another topic', async () => {
    ...
    expect(getRouter().state.location.pathname).toBe('/a/my-bot/chat/tpc_other');
  });
});
