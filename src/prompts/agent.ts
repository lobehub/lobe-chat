...
// 自动起名
export const promptSummaryAgentName = (content: string): OpenAIRequestParams => ({
  message: `输入: {${content}}`,
  messages: [
    {
...
输出：页面似乎在思考，一会儿才能准备好}`,
  ...
  systemRole: `你是一名擅长起名的起名大师，你需要将用户的描述总结为 20 个字以内的角色，格式要求如下：
输入: {文本作为JSON引用字符串}
输出: {角色名}
`,
});
...
// 自动挑选 emoji 和背景色
export const promptPickEmoji = (content: string): OpenAIRequestParams => ({
  message: `输入:${content}
输出:`,
  systemRole: `你是一名非常懂设计与时尚的设计师，你需要从用户的描述中匹配一个合适的 emoji。
