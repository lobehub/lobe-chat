export default {
  ...
  analytics: {
    telemetry: {
      desc: '通过选择发送遥测数据，你可以帮助我们改善 EDUGPT 整体用户体验',
      title: '发送匿名使用数据',
    },
    title: '数据统计',
  },
  ...
  llm: {
    ...
    customModelCards: {
      ...
      modelConfig: {
        ...
        files: {
          extra:
            '当前 EDUGPT 的文件上传实现仅为一种 Hack 方案，仅限自行尝试。完整文件上传能力请等待后续实现',
          title: '支持文件上传',
        },
        functionCall: {
          extra:
            '此配置将仅开启 EDUGPT 中的函数调用能力，是否支持函数调用完全取决于模型本身，请自行测试该模型的函数调用能力可用性',
          title: '支持函数调用',
        },
        id: {
          extra: '将作为模型标签进行展示',
        ...
        vision: {
          extra:
            '此配置将仅开启 EDUGPT 中的图片上传配置，是否支持识别完全取决于模型本身，请自行测试该模型的视觉识别能力可用性',
          title: '支持视觉识别',
        },
      },
    },
