...
describe('modelParse', () => {
    ...
    describe('processModelList', () => {
        ...
        describe('Detailed capability and property processing in processModelList', () => {
            ...
            it('should prioritize model.displayName > knownModel.displayName > model.id', async () => {
                ...
                expect(result.find(m => m.id === 'model-known-displayname')!.displayName).toBe('Known Model DisplayName');
    ...
    describe('MODEL_LIST_CONFIGS and PROVIDER_DETECTION_CONFIG', () => {
        ...
        it('should have proper structure for each provider config in MODEL_LIST_CONFIGS', () => {
            for (const providerConfig of Object.values(MODEL_LIST_CONFIGS)) {
                expect(providerConfig).toHaveProperty('excludeKeywords');
                expect(Array.isArray(providerConfig.excludeKeywords)).toBe(true);
                expect(providerConfig).toHaveProperty('functionCallKeywords');
                expect(Array.isArray(providerConfig.functionCallKeywords)).toBe(true);
                expect(providerConfig).toHaveProperty('reasoningKeywords');
                expect(Array.isArray(providerConfig.reasoningKeywords)).toBe(true);
                expect(providerConfig).toHaveProperty('visionKeywords');
                expect(Array.isArray(providerConfig.visionKeywords)).toBe(true);
            }
        });
    });
});