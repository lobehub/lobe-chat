...
export const PollinationsManifest: BuiltinToolManifest = {
  api: [
    {
      description: 'Create images from a text-only prompt using Pollinations.AI.',
      name: 'text2image',
      parameters: {
        properties: {
          prompts: {
            description:
              "The user's original image description, potentially modified to abide by the lobe-pollinations-designer policies. If the user does not suggest a number of captions to create, create four of them. If creating multiple captions, make them as diverse as possible. If the user requested modifications to previous images, the captions should not simply be longer, but rather it should be refactored to integrate the suggestions into each of the captions. Generate no more than 4 images, even if the user requests more.",
            items: {
              type: 'string',
            },
            maxItems: 4,
  ...
  identifier: 'lobe-pollinations-designer',
  meta: {
  ...
  systemRole: `Whenever a description of an image is given, use lobe-pollinations-designer to create the images and then summarize the prompts used to generate the images in plain text. If the user does not ask for a specific number of images, default to creating four captions to send to lobe-pollinations-designer that are written to be as diverse as possible.

  All captions sent to lobe-pollinations-designer must abide by the following policies:
...
All descriptions sent to lobe-pollinations-designer should be a paragraph of text that is extremely descriptive and detailed. Each should be more than 3 sentences long.`,
  type: 'builtin',
};
