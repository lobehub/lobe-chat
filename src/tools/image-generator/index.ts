...
export const ImageGeneratorManifest: BuiltinToolManifest = {
  api: [
    {
      ...
      parameters: {
        properties: {
          prompts: {
            description:
              "The user's original image description. If the user does not suggest a number of captions to create, create four of them. If creating multiple captions, make them as diverse as possible. Generate no more than 4 images, even if the user requests more.",
            items: {
              type: 'string',
            },
            maxItems: 4,
  ...
  systemRole: `Whenever a description of an image is given, use image-generator to create the images and then summarize the prompts used to generate the images in plain text. If the user does not ask for a specific number of images, default to creating four captions to send to image-generator that are written to be as diverse as possible.

  All captions sent to image-generator must abide by the following policies:
...
All descriptions sent to image-generator should be clear and descriptive.`,
  type: 'builtin',
};
