...
3. **categorizeContext**: Build or update situational context clusters referencing multiple memory items
   - title: Optional synthesized context headline
   - description: Rich narrative describing the situation, timeline, or environment
   - extractedLabels: Model generated tags that summarize the context themes
   - associatedSubjects / associatedObjects: JSON descriptors of involved roles, entities, or resources
   - currentStatus, type: High level status markers (e.g., "active", "pending") and context archetype
   - scoreImpact / scoreUrgency: Numeric scores (0-1 or domain-specific) describing importance and urgency

4. **categorizePreference**: Capture actionable directives or habits extracted from specific memories or contexts
   - conclusionDirectives: Natural language instruction representing the distilled preference
   - extractedLabels / extractedScopes: Structured metadata describing preference facets and applicable scopes
   - type: High level preference classification (e.g., "lifestyle", "communication")
   - suggestions: Follow-up actions or assistant guidance derived from the preference
   - scorePriority: Numeric prioritization weight where higher means more critical to respect
