...
export const getServerGlobalConfig = async () => {
  ...
  if (isDev) {
    console.log('⏳ [Server] Loading global config...');
  }
  ...
  const aiProviderTime = Date.now() - aiProviderStartTime;
  if (isDev) {
    console.log(`✅ [Server] AI Provider config loaded in ${aiProviderTime}ms`);
  }
  ...
  if (isDev) {
    console.log(`✅ [Server] Global config loaded in ${totalTime}ms`);
  }
