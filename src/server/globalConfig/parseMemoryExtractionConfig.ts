...
): MemoryAgentConfig | undefined => {
  const model = process.env.MEMORY_USER_MEMORY_EMBEDDING_MODEL;

  if (!model) return undefined;

  return {
    apiKey: process.env.MEMORY_USER_MEMORY_EMBEDDING_API_KEY,
    baseURL: process.env.MEMORY_USER_MEMORY_EMBEDDING_BASE_URL,
    model: model || fallbackModel,
    provider: process.env.MEMORY_USER_MEMORY_EMBEDDING_PROVIDER || fallbackProvider,
  };
};
