...
export const configRouter = router({
  ...
  getGlobalConfig: publicProcedure.query(async ({ ctx }): Promise<GlobalRuntimeConfig> => {
    console.log('[GlobalConfig] Starting global config retrieval for user:', ctx.userId || 'anonymous');
    
    const serverConfig = await getServerGlobalConfig();
    console.log('[GlobalConfig] Server config retrieved');
    
    const serverFeatureFlags = await getServerFeatureFlagsStateFromEdgeConfig(ctx.userId || undefined);
    console.log('[GlobalConfig] Final feature flags to return (evaluated booleans only):', serverFeatureFlags);

    const result = { serverConfig, serverFeatureFlags };
    console.log('[GlobalConfig] Returning global config with feature flags keys:', Object.keys(serverFeatureFlags));
