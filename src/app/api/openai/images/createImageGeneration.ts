...
// ];
// export const createImageGeneration=async () => new Response(JSON.stringify([mockImages[Math.round(Math.random() * 3)]]));
...
export const createImageGeneration = async ({
  ...
  // return new Response(JSON.stringify([mockImages[Math.round(Math.random() * 3)]]));
