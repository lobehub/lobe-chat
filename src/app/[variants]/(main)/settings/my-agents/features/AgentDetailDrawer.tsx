...
const AgentDetailDrawer = memo<AgentDetailDrawerProps>(
  ({ open, onClose, agent, onStatusChange }) => {
    ...
    const handleStatusAction = useCallback(
      (action: AgentStatusAction) => {
        if (!agent?.identifier || !onStatusChange) return;
        ...
        if (action === 'deprecate') {
          modal.confirm({
            ...
            onOk: () => onStatusChange(agent.identifier, action),
            title: t('myAgents.actions.deprecateConfirmTitle'),
          });
        } else {
          onStatusChange(agent.identifier, action);
        }
      },
      [agent?.identifier, onStatusChange, modal, t],
    );

    // Determine if agent is currently published based on status
        ...
        <Flexbox className={styles.actions} gap={12} padding={16}>
              ...
              items: [
                {
                  ...
                  key: 'deprecate',
                  label: t('myAgents.actions.deprecate'),
          ...
          >
            <Flexbox align="center" gap={4} horizontal justify="center">
              <Icon icon={isPublished ? EyeOff : Eye} />
              {isPublished ? t('myAgents.actions.unpublish') : t('myAgents.actions.publish')}
            </Flexbox>
          </Dropdown.Button>
        </Flexbox>
      </Drawer>
