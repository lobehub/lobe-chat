...
const ICON_ACTIVE_SIZE: ActionIconProps['size'] = {
  blockSize: 40,
  size: 24,
  strokeWidth: 2,
};
    ...
    <Flexbox gap={8}>
      ...
      >
        <ActionIcon
          active={isChatActive}
          icon={MessageSquare}
          size={isChatActive ? ICON_ACTIVE_SIZE : ICON_SIZE}
          title={t('tab.chat')}
      ...
      {enableKnowledgeBase && (
        <Link aria-label={t('tab.files')} href={'/files'}>
          <ActionIcon
            ...
            icon={FolderClosed}
            size={isFilesActive ? ICON_ACTIVE_SIZE : ICON_SIZE}
            title={t('tab.files')}
            tooltipProps={{ placement: 'right' }}
          />
        </Link>
      ...
      {showMarket && (
        <Link aria-label={t('tab.discover')} href={'/discover'}>
          <ActionIcon
            ...
            icon={Compass}
            size={isDiscoverActive ? ICON_ACTIVE_SIZE : ICON_SIZE}
