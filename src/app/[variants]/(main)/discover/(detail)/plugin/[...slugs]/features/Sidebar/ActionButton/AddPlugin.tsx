...
const AddPlugin = memo(() => {
  ...
  const [useFetchPluginStore, installed, installing, installPlugin, unInstallPlugin] = useToolStore(
    (s) => [
      ...
      pluginSelectors.isPluginInstalled(identifier as any)(s),
      pluginStoreSelectors.isPluginInstallLoading(identifier as any)(s),
      s.installPlugin,
      s.uninstallPlugin,
    ],
  );
  ...
  const handleInstall = async () => {
    setLoading(true);
    await installPlugin(identifier as any);
    setLoading(false);
  };
  ...
  const handleUninstall = async () => {
    modal.confirm({
      ...
      onOk: async () => {
        setLoading(true);
        await unInstallPlugin(identifier as any);
        setLoading(false);
      },
