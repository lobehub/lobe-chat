...
export class MemoryExtractAgentService {
  ...
  async run(job: MemoryExtractionJob): Promise<MemoryExtractionResult | null> {
    ...
    try {
      const decision = await this.runGatekeeper(context);
  ...
  private async runGatekeeper(context: PreparedExtractionContext): Promise<GatekeeperDecision> {
    return this.gatekeeper.check(context.conversation, {
  ...
  private async persistLayers(
    ...
    for (const layer of layers) {
      switch (layer) {
        case 'context': {
          const result = await this.contextExtractor.extract(context.conversation, context.options);
        ...
        case 'experience': {
          const result = await this.experienceExtractor.extract(
        ...
        case 'preference': {
          const result = await this.preferenceExtractor.extract(
            context.conversation,
            context.options,
          );
        ...
        case 'identity': {
          ...
          const result = await this.identityExtractor.extract(
            context.conversation,
            identityOptions,
          );
  ...
  private async persistContextMemories(
    ...
    for (const item of result.memories ?? []) {
      ...
      const { memory } = await userMemoryModel.createContextMemory({
        context: {
          associatedObjects: item.withContext?.associatedObjects ?? null,
          associatedSubjects: item.withContext?.associatedSubjects ?? null,
