...
const getExistingState = (metadata?: ChatTopicMetadata | null) => {
  return metadata?.memoryExtraction?.sources?.[SOURCE_KEY] ?? null;
};
...
export class ChatTopicProvider implements MemoryExtractionProvider {
  ...
  async complete(
    ...
    const memoryExtraction = existingMetadata.memoryExtraction ?? {
      lastRunAt: undefined,
      sources: {},
    };
    ...
    memoryExtraction.sources = {
      ...memoryExtraction.sources,
      [SOURCE_KEY]: sourceState,
    };
