...
import type { FtsSearchRepo, FtsSearchResult } from '../index';

export const userId = 'search-test-user';
export const otherUserId = 'other-search-user';
...
export interface FtsSearchProductBehaviorOptions {
  /** Builds the repo under test for a given scope. */
  createRepo: (
    db: LobeChatDatabase,
    userId: string,
    workspaceId?: string,
 ...
 * expectations. Provider-specific guards (emitted SQL shape, index mappings)
 * stay in the caller's own test file.
 */
export const describeFtsSearchProductBehavior = (options: FtsSearchProductBehaviorOptions) => {
  const { createRepo } = options;
  const serverDB = options.db;
  ...
  describe('searchKnowledgeBaseDocuments', () => {
    ...
    it('should hide caller-private workspace documents from a public agent', async () => {
      ...
      const privateAgentResults = await createRepo(
        serverDB,
        userId,
        workspaceId,
        'private',
      ).searchKnowledgeBaseDocuments('confidential launch', [privateKbId]);
      const publicAgentResults = await createRepo(
        serverDB,
        userId,
        workspaceId,
        'public',
  ...
  describe('search - workspace scoping', () => {
    ...
    it('should never surface workspace-scoped rows in personal mode', async () => {
      const results = await createRepo(serverDB, userId).search({ query: 'Kubernetes' });

      expectEveryHitScopedTo(results, 'personal');
    });

    it('should never surface personal rows in workspace mode', async () => {
      const results = await createRepo(serverDB, userId, workspaceId).search({
        query: 'Kubernetes',
      });
