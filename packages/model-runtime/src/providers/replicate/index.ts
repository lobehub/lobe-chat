...
export class LobeReplicateAI implements LobeRuntimeAI {
  ...
  async textToImage(payload: TextToImagePayload): Promise<string[]> {
    try {
      ...
      if (model.includes('flux') && // Convert size to aspect ratio (e.g., "1024x1024" -> "1:1")
        size) {
      ...
      // Run prediction - with useFileOutput: false, returns URL strings
      console.log('[Replicate] Calling client.run with:', JSON.stringify({ input, model }));
      const output = await this.client.run(model as any, { input });
      console.log('[Replicate] Output:', output);
      console.log('[Replicate] Output type:', typeof output, 'Is array:', Array.isArray(output));

      // Extract URLs from output
      if (Array.isArray(output)) {
        const urls = output.filter((item) => typeof item === 'string');
        console.log('[Replicate] Filtered URLs:', urls);
        return urls.slice(0, n);
      } else if (typeof output === 'string') {
        console.log('[Replicate] Single URL:', output);
        return [output];
      }
