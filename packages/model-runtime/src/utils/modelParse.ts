...
export interface ModelProcessorConfig {
  excludeKeywords?: readonly string[]; // Do not add labels to models that match these keywords
...
// Default keywords: any model ID containing -search is treated as supporting web search
const DEFAULT_SEARCH_KEYWORDS = ['-search'] as const;

// Model capability label keyword configuration
...
/**
 * Detects whether the keyword list matches the model ID (supports multiple matching modes)
...
const isKeywordListMatch = (modelId: string, keywords: readonly string[]): boolean => {
  ...
  // If any exclusion rule matches, return false immediately
  ...
  return includeKeywords.some((keyword) => {
    if (keyword.startsWith('^')) {
      // If starts with ^, match only at the beginning
...
/**
 * Finds the corresponding local model configuration based on the provider type
...
/**
 * Detects the provider type of a single model
 * @param modelId Model ID
...
const formatTimestampToDate = (timestamp: number): string | undefined => {
  ...
  // Supports second-level or millisecond-level timestamps:
  // - If millisecond-level (>= 1e12), treat directly as milliseconds;
...
/**
 * Processes the releasedAt field
...
/**
 * Processes the model display name
...
/**
 * Gets the local configuration for a model provider
...
/**
 * Gets the local configuration for a model
...
/**
 * Processes the model list for a single provider
 * @param modelList Model list
 * @param config Provider configuration
 * @param provider Provider type (optional, used to prioritize matching the corresponding local configuration, only when provider is provided will it attempt to override enabled from local configuration)
...
export const processModelList = async (
  ...
  // If provider is provided, attempt to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If provider is provided and has local configuration, attempt to get the model's enabled status from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );

      // If found the model in local configuration, use its enabled status
      if (
        processedModel &&
...
/**
 * Processes the model list for multiple providers
 * @param modelList Model list
...
export const processMultiProviderModelList = async (
  ...
  // If providerid is provided, attempt to get the local configuration for that provider
  ...
  return Promise.all(
    modelList.map(async (model) => {
      ...
      // If providerid is provided and has local configuration, attempt to get the model's enabled status from it
      const providerLocalModelConfig = getModelLocalEnableConfig(
        providerLocalConfig as any[],
        model,
      );
