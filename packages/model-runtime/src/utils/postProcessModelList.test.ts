...
describe('isAutoGeneratedImageModelId', () => {
  it('should identify image model ids generated from whitelisted base models', () => {
    expect(isAutoGeneratedImageModelId('gemini-3.1-flash-image-preview:image')).toBe(true);
    expect(isAutoGeneratedImageModelId('custom-gemini-2.5-flash-image-preview:image')).toBe(true);
    expect(isAutoGeneratedImageModelId('gemini-2.5-flash-image-preview:free:image')).toBe(true);
  });

  it('should reject base models and non-whitelisted image model ids', () => {
    expect(isAutoGeneratedImageModelId('gemini-3.1-flash-image-preview')).toBe(false);
    expect(isAutoGeneratedImageModelId('custom-image-model:image')).toBe(false);
  });
});
