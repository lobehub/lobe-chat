import type { BuiltinSkillManifest } from '@lobechat/types';

export const ArtifactsIdentifier = 'lobe-artifacts';

export const ArtifactsManifest: BuiltinSkillManifest = {
  avatar: `data:image/svg+xml;base64,`,
  description:
    'Create directly viewable web pages, browser games, SVG graphics or animations, and React components. Prefer this skill for visual works, including requests to show the result directly or preview it. Use files when the user explicitly requests a download. Use inline preview only when the user explicitly asks to display inside the chat message or code block, or rejects a separate Artifact panel; respond with a complete HTML code block (wrap SVG inside it) without activation. Naming HTML or SVG alone does not request a file. Self-contained previews need no sandbox activation; answer code snippets and explanations directly without loading this skill.',
  identifier: ArtifactsIdentifier,
  name: 'artifacts',
  source: 'builtin',
};
