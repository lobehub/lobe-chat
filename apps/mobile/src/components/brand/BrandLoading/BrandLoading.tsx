...
import type { BrandLoadingProps } from './type';

// 创建 Animated 版本的 Path (使用 React Native 的 Animated)
const AnimatedPath = Animated.createAnimatedComponent(Path);

const BrandLoading = memo<BrandLoadingProps>(({ size = 64, style, color, ...rest }) => {
  // 使用 React Native 的 Animated.Value
  const strokeDashoffset = useRef(new Animated.Value(1000)).current;
  const fillOpacity = useRef(new Animated.Value(0)).current;
  const theme = useTheme();

  useEffect(() => {
    // 描边绘制动画：从 1000 到 0 循环
    const drawAnimation = Animated.loop(
      Animated.sequence([
        Animated.timing(strokeDashoffset, {
          duration: 2000,
          easing: Easing.bezier(0.4, 0, 0.2, 1),
          toValue: 0,
          useNativeDriver: false, // SVG 属性不能使用 native driver
        }),
        Animated.timing(strokeDashoffset, {
          duration: 0,
          toValue: 1000, // 立即重置
    ...
    const fillAnimation = Animated.loop(
      Animated.sequence([
        Animated.delay(400), // 稍微延迟，让描边先出现
  ...
  return (
    ...
    >
      <AnimatedPath
        ...
        fill={color || theme.colorText}
        fillOpacity={fillOpacity}
        fillRule="evenodd"
        stroke={color || theme.colorText}
