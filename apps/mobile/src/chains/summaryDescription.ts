...
export const chainSummaryDescription = (content: string): Partial<ChatStreamPayload> => ({
  messages: [
    {
      content: `你是一名擅长技能总结的助理，你需要将用户的输入的内容总结为一个角色技能简介，不超过 20 个字。内容需要确保信息清晰、逻辑清晰，并有效地传达角色的技能和经验，需要并翻译为目标语言:${i18n.language}。格式要求如下：\n输入: {文本作为JSON引用字符串} [locale]\n输出: {简介}`,
      role: 'system',
    },
    {
...
---\n\n\`useWatchPluginMessage\` 是 Chat Plugin SDK 封装一个的 React Hook，用于监听从 LobeChat 发过来的插件消息。
    ...
    { content: `输入: {${content}} [${i18n.language}]`, role: 'user' },
  ],
  temperature: 0,
});
