...
<Steps>
  ...
  Add a new lobe-chat item under identity\_providers -> oidc:
  ...
  Make sure to replace secret and redirect\_urls with your own values. Note! The secret configured in Authelia is ciphertext, i.e., a salted hash value. Its corresponding plaintext needs to be filled in LobeChat later.
  ...
  | `AUTH_AUTHELIA_SECRET`    | Required | The plaintext corresponding to the secret just configured in Authelia, example value is insecure\_secret                                                                                                                                                  |
