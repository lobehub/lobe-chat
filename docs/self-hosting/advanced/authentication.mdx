...
<Callout type={'info'}>
  For information on using the Next-Auth with a server-side database, please refer to [Deploying with a Server Database](/docs/self-hosting/advanced/server-database).
</Callout>
