...
使用 Zeabur 平台一键部署 WebRTC 信令服务器： [![Deploy on Zeabur](https://zeabur.com/button.svg)](https://zeabur.com/templates/MY0JZG?referralCode=arvinxx)
...
LobeChat 默认隐藏了 WebRTC 同步功能，需要通过配置环境变量 `FEATURE_FLAGS=+webrtc_sync` 开启 WebRTC 同步的特性。

### 配置 LobeChat 的 WebRTC 同步设置

- 打开 LobeChat 设置
- 在同步设置中添加第一步中获得的信令服务器地址；
- 设置同步频道名称和密码
