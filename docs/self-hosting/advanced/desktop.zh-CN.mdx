...
2. 对于 Vercel 部署，请确保 `APP_URL` 环境变量设置为您当前配置的 URL，这对于正确处理认证回调至关重要
...
- 如果您使用第三方认证服务（如 Clerk），请在认证服务的配置中添加桌面端的回调 URL
- 更新到最新版本的桌面端（1.83.6 或更高）
- 对于 Vercel 部署，检查 `APP_URL` 环境变量是否与您访问的 URL 一致
...
- 检查您是否下载了正确的 macOS 架构版本：
  - 对于 Apple Silicon (M1/M2/M3) Mac，应使用 `arm64` 版本
