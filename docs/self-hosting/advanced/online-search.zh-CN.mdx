...
设置单页抓取内容的字符上限。用于控制爬虫在抽取网页正文时的最大内容量，避免过长内容带来的内存与响应时间开销。默认值：25000。

```env
CRAWL_CONTENT_LIMITED_COUNT=25000
```

> 📌 建议根据实际需求调整此值，避免抓取过长内容导致性能问题。

---

## `SEARCH_ITEM_LIMITED_COUNT`

设置每次联网搜索返回的结果条目上限。用于在搜索阶段限制候选结果数量，从而控制后续抓取与总结的工作量。默认值：30。

```env
SEARCH_ITEM_LIMITED_COUNT=30
```

> 📌 建议范围：10～50。过小可能影响召回率，过大将明显增加抓取与处理开销。
