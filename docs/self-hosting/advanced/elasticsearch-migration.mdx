...
Promotion refuses to run while the Outbox has work, because the consumer must have written every
change to the new generation first.
...
Retirement is two-phase and preserves indexes still attached to the entity's alias. The first run
closes detached old indexes; a later run deletes indexes that are already closed. Before the deletion
run, let every pre-close drain finish and confirm that a fresh drain resolves targets without the
closed indexes. Do not run the command twice back-to-back: the CLI does not enforce this barrier,
and a consumer holding an old target list can recreate a deleted index with a dynamic mapping.
