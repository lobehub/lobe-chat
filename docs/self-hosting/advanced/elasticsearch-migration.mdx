...
Retirement is two-phase. The first run closes the old index, which removes it from the consumer's
write targets; a later run deletes indexes that are already closed. Closing first avoids a window in
which a consumer that still lists the deleted index recreates it with a dynamic mapping.
