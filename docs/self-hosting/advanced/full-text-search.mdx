...
description: Choose and operate the pg_search, Elasticsearch, or pg_like backend for LobeHub product search.
tags:
  - Full-Text Search
  - Elasticsearch
  - pg_search
...
LobeHub supports three product-search backends:
...
| `pg_like`       | A small deployment on plain PostgreSQL that cannot install `pg_search` and does not want to run Elasticsearch     | None; substring matching runs directly against the existing tables without extensions, indexes, or sync workers                                                                                                          |

`FTS_SEARCH_PROVIDER` selects one backend for the whole deployment. The accepted values are
`pg_search`, `elasticsearch`, and `pg_like`; the default is `pg_search`. LobeHub does not silently
fall back to another backend when the selected provider is unavailable.
...
### Use `pg_like` on plain PostgreSQL

Choose `pg_like` when your PostgreSQL service cannot install `pg_search` (many managed providers) and
the deployment is small enough that a dedicated search service is not worth operating. It needs no
extension, no additional index, and no sync worker: every search runs as case-insensitive substring
matching (`ILIKE`) over the same tables and permission scopes as `pg_search`.

```bash
FTS_SEARCH_PROVIDER=pg_like
```

What to expect compared with the other backends:

- **Matching.** A row matches when every word of the query appears in one searchable field. There is
  no stemming, no typo tolerance, and no phrase proximity. Chinese, Japanese, and Korean text works
  without segmentation because matching is substring based.
- **Ranking.** Results are ordered by a simple relevance score: a whole-field match ranks above a
  prefix match, which ranks above a phrase match, which ranks above rows that merely contain every
  word; title-like fields weigh more than body text.
- **Performance.** Queries scan the scoped rows instead of using a search index. This is fine for
  personal and small-team instances; for large message or memory tables, prefer `pg_search` or
  Elasticsearch.

The same migration limitation as Elasticsearch applies: `pg_like` does not remove the historical
`pg_search` migrations, so a fresh database still has to run on an image that provides the
extension until those migrations become optional.
...
| Variable                  | Purpose                                                                                                                                                       |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FTS_SEARCH_PROVIDER`     | Deployment-wide provider: `pg_search`, `elasticsearch`, or `pg_like`                                                                                          |
