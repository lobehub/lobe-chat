...
以下两个命令都需要 `ES_URL` 和当前部署的 Elasticsearch 认证配置，包括 `--skip-failure`：它在修改 checkpoint 前也会获取同一命名空间的迁移锁。请保留前面配置并导出的这些变量。
...
`--status` 是只读操作。`mappingDiff` 列出新增、删除、修改的字段及分析器变化；各个打开的索引通过 `fieldCompatibility` 显示文档兼容性，`retirementPlan` 列出关闭、删除候选和阻塞原因。

### 保留回滚所需的源字段

从新映射删除字段时，旧索引可能仍然需要它。回滚窗口内，在 Zod schema 和 builder 中继续产出旧字段，并在 `ftsSearchDocument/policy.ts` 的 `FTS_SEARCH_RETAINED_SOURCE_PROPERTIES` 中声明它的旧属性。全量回填写入新索引前会过滤这些保留字段，增量同步仍会把它们写入旧映射。等所有需要旧字段的索引关闭、且不再需要回滚旧代码之后，才能删除保留字段。字段改名时同样需要在过渡期同时产出新旧字段。

在线迁移不支持在同一个字段名下改成不兼容的 JSON 值类型。例如，把 keyword 类型的 `priority` 改为整数时，新增 `priority_rank`，继续以保留源字段产出旧 `priority`，切换索引后再把查询改为新字段名。仅升版本并重建索引不能提供值转换能力。

如果当前投影无法表示某个打开索引所需的字段或值类型，同步会拒绝写入。检查针对字段定义，不要求每个可选值都非空；text 和 keyword 可以互认字符串，其他类型变化需要兼容的投影。这项保护不会转换历史值，也无法判断字段业务含义是否变化。

受影响实体的记录会保留为带错误说明的死信，其它兼容实体可以完成本轮消费。独立同步 CLI 遇到死信会退出，后续运行也会拒绝启动，直到死信修复。需要监控增量队列的 `dead` 数量；修正投影或退役不兼容索引后，显式修复或重新入队这些记录，仅部署修复不会自动重试死信。这里与 `--skip-failure` 不同，后者只处理全量回填 checkpoint 中的失败。

目前没有 Outbox 重新入队的 CLI 参数。修复原因并通过 `--status` 确认所有打开目标的 `fieldCompatibility` 兼容后，使用 PostgreSQL 管理客户端连接源数据库，先检查受影响记录：
...
把下面的文档 ID 和修订号替换为查询所得的配对值，只重新入队这些死信。保留原修订号，防止迟到写入覆盖更新的源数据。匹配已检查的修订号，还能避免误重置检查期间被源数据变更恢复、随后又失败的新任务：
...
没有出现在 UPDATE 返回结果中的记录应重新检查，不能猜测新修订号重试。逐一处理其它已检查的死信，再启动同步 CLI，验证目标文档和 `--status`。不要清空整个队列，也不要只为重新触发采集而修改源记录。这项数据库修复本身不能修正不兼容的 mapping。

### 同一命名空间只运行一个迁移进程

变更命令会在 Elasticsearch 的 `<namespace>-fts-search-control` 中获取不自动过期的锁，覆盖指向同一集群和命名空间的不同进程与检查点目录。迁移 key 需要对此控制索引具有创建、读取、写入和删除权限。增量同步不持有这个锁，仍可正常运行。命令成功后释放锁；失败或中断时可能保留锁，因为超时的 Elasticsearch 请求仍可能在服务端完成。

获取锁后的失败统一保留锁，也包括尚未写入时的校验失败。仅修正输入不会释放这把锁，需要按下面的步骤显式恢复。

通过 `--status` 的 `migrationLock` 查看持有者。先停止旧进程并确认结果不明的请求已结束，再显式释放遗留锁；owner UUID 只是标识，不代表旧进程已停止：

```bash
bun run fts-search:reindex -- --release-lock=<owner-uuid> --yes
```

锁不会超时后自动交给下一进程。不要同时运行绕过这项保护的旧版 CLI；CLI 之外的人工操作也不受该锁约束。
...
等 `--status` 显示目标实体的增量队列空闲（`pending`、`retrying`、`inFlight`、`dead` 都为 `0`）后再切换，其它实体的任务不阻塞这次别名切换：
...
重复执行已经完成的切换，在身份校验通过后返回 `outcome: already_live`，不修改别名，也不要求此时新增的增量工作再次清空。
...
`--retire` 只关闭已脱离别名的旧索引，重复执行也不会删除数据。永久删除符合条件的已关闭索引需要单独执行：
...
删除前，命令会为精确的旧索引名安装并验证 `allow_auto_create: false` 模板，迁移 key 需要 `manage_index_templates` 权限。保留该模板可以阻止迟到的 bulk 请求重新创建已删除索引；仅凭队列清空无法证明暂停的旧进程已停止。模板冲突或无法验证保护时拒绝删除。删除之后及恢复集群状态时都要保留保护模板。它不能阻止显式创建索引，也不能防止人工替换模板，因此不要再用旧版 CLI 重建已清理的版本。
...
如果更新 mapping 后、建立检查点前中断，处理遗留锁后可以重跑同一条 `--apply --in-place` 命令。只有元数据和映射都匹配时才允许继续；仅看到 `in_sync` 不代表回填已经完成。
