...
机器人发送附件时也使用此白名单，并校验每次重定向及所有 DNS 解析结果。条目必须是完整 IP 地址，不支持主机名或 CIDR 网段。例外允许访问该 IP 的不同端口和路径，请仅填写允许机器人工具访问的服务器。附件下载器不会通过 `SSRF_ALLOW_PRIVATE_IP_ADDRESS` 开启任意内网访问。
