...
- Default: `-`
- Example: `+qwen-7b-chat,+glm-6b,-gpt-3.5-turbo,gpt-4-turbo=gpt-4o`

The above example adds `qwen-7b-chat` and `glm-6b` to the model list, removes `gpt-3.5-turbo` from the list, and displays the name of `gpt-4-turbo` as `gpt-4o`. If you want to disable all models first and then enable specific models, you can use `-all,+gpt-3.5-turbo`, which means only `gpt-3.5-turbo` will be enabled.
...
Use the official [Meta Model API](https://dev.meta.ai/). See [Using Meta in LobeHub](/docs/usage/providers/meta) for setup instructions.

### `META_API_KEY`

- Type: Optional for the deployment; required when providing a shared server key. Users can instead enter their own key in Meta provider settings.
- Description: A key created in the [Model API dashboard](https://dev.meta.ai/). Setting a non-empty key automatically enables Meta in the server configuration. `ENABLED_META` is derived from this key, not a separate environment switch. LobeHub uses `META_API_KEY`, rather than the `MODEL_API_KEY` name in Meta's SDK examples.
- Default: Unset; no shared server key.
- Example: `your-meta-model-api-key`

### `META_PROXY_URL`

- Type: Optional
- Description: Overrides the Meta API base URL. The proxy must support the Responses API. Supply the base URL, not the full `/responses` endpoint.
- Default: `https://api.meta.ai/v1`
- Example: `https://meta-proxy.example.com/v1`

### `META_MODEL_LIST`

- Type: Optional
- Description: Customizes the model list using the shared [model-list syntax][model-list]: `+` adds a model, `-` hides a model, and `model_id=display_name` renames a model. Separate entries with commas. This does not grant access to models unavailable to your Meta account.
- Default: Unset; uses the built-in model list.
- Example: `-all,+muse-spark-1.3`

The example hides all default models and adds `muse-spark-1.3`. See the [Muse Spark product page](https://developer.meta.com/ai/models/muse-spark/) and Meta's [Muse Spark 1.3 announcement](https://www.threads.com/@metafordevelopers/post/Dcy9DWLmfVn) for version information. Model availability depends on your account's API access.
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+gemini-2.5-flash,+gemini-2.5-pro`
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+accounts/fireworks/models/llama-v3p3-70b-instruct,+accounts/fireworks/models/deepseek-v3`
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+llama-3.3-70b-versatile,+llama-3.1-8b-instant`
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+glm-4.6,+glm-4-plus`
...
- Example: `https://my-qiniu-proxy.com/v1`
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+meta-llama/llama-3.1-8b-instruct,+meta-llama/llama-3.3-70b-instruct`
...
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+hunyuan-2.0-instruct-20251111,+hunyuan-2.0-thinking-20251109`
...
### `JALAPENOCLOUD_API_KEY`

- Type: Required
- Description: API key from [Jalapeno Cloud 50%off](https://www.jalapeno-cloud.ai)
- Default: -
- Example: `sk-xxxxxx...xxxxxx`

### `JALAPENOCLOUD_MODEL_LIST`

- Type: Optional
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules see [model-list][model-list]
- Default: `-`
- Example: `-all,+DeepSeek-V4-Pro`

### `JALAPENOCLOUD_PROXY_URL`

- Type: Optional
- Description: If you manually configure the Jalapeno Cloud 50%off interface proxy, you can use this configuration item to override the default Jalapeno Cloud 50%off API request base URL
- Default: `https://api.jalapeno-cloud.ai/v1`
- Example: `https://api.jalapeno-cloud.ai/v1`
...
Connect to your own Unsloth Studio instance. See [Using Unsloth in LobeHub](/docs/usage/providers/unsloth) for browser authentication and desktop CORS limitations.

### `UNSLOTH_API_KEY`

- Type: Optional for the deployment; set a real Studio key when providing authenticated server access. Users can instead configure their key in provider settings.
- Description: The API key for your Studio instance. A non-empty value enables the provider in server configuration. `ENABLED_UNSLOTH` is derived from this key and is not an independent environment switch. Keyless local server access depends on Studio's access policy; cross-origin browser requests require valid authentication.
- Default: Unset; no shared server key.
- Example: `your-unsloth-studio-api-key`

### `UNSLOTH_PROXY_URL`

- Type: Optional
- Description: Overrides the OpenAI-compatible API base URL. Use an address reachable from the LobeHub server. Inside Docker, `127.0.0.1` refers to the container, not the host running Studio. An explicitly configured Studio port overrides its default `8888`.
- Default: `http://127.0.0.1:8888/v1`
- Example: `http://studio.example.com:8888/v1`

### `UNSLOTH_MODEL_LIST`

- Type: Optional
- Description: Customizes the model list using the shared [model-list syntax][model-list]: `+` adds a model, `-` hides a model, and `model_id=display_name` renames a model. Separate entries with commas. Use IDs returned by your Studio instance; this setting does not download models.
- Default: Unset; uses the built-in example model list with the examples disabled. This does not guarantee local availability or change existing saved model selections.
- Example: `-all,+your-local-model-id=Local Model`
