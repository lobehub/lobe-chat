...
| Numeric Value | `params.max_tokens=300`, `enableHistoryCount=5` | Set the maximum tokens to `300`, Set the number of historical messages to 5. |
| Boolean Value | `enableAutoCreateTopic=true`, `enableCompressThreshold=true`, `enableHistoryCount=true` | Enable automatic topic creation, History length compression threshold, number of historical records. |
