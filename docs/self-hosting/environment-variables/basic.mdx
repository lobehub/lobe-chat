...
Outbound bot attachments also honor this list, including each redirect and DNS answer. Entries are exact IP addresses, not hostnames or CIDR ranges. An exception allows requests to that IP across ports and paths; list only servers you intend bot tools to access. The attachment downloader does not enable unrestricted private access via `SSRF_ALLOW_PRIVATE_IP_ADDRESS`.
