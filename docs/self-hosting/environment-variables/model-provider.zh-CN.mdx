...
- 默认值：`http://localhost:8188`
- 示例：`http://192.168.1.100:8188` 或 `https://my-comfyui-server.com`
...
<Callout type={'info'}>
  **智能认证管理**: 系统内置 AuthManager 类，支持自动认证验证和错误恢复。认证失败（401/404 错误）会自动触发重新配置界面。
</Callout>

<Callout type={'warning'}>
  **认证错误自动恢复**: 当发生认证配置错误（密码错误、API 密钥无效、baseURL 错误）时，系统会自动检测并弹出认证配置界面让您重新输入，无需手动刷新页面。
</Callout>

### 模型支持和架构

ComfyUI 集成现已支持 **223 个模型**，包含完整的模型注册表系统：

#### 模型统计

- **FLUX 系列**: 130 个模型（官方 4 个 + 企业 106 个 + 社区 20 个）
- **SD 系列**: 93 个模型（SD1.5、SDXL、SD3.5）
- **系统组件**: 39 个（VAE、CLIP、T5、LoRA、ControlNet）

#### 支持的模型 ID

- **FLUX 系列**: `flux-dev`, `flux-schnell`, `flux-kontext-dev`, `flux-krea-dev`
- **SD3.5 系列**: `stable-diffusion-35`, `stable-diffusion-35-inclclip`
- **SD1.5/SDXL**: `stable-diffusion-15`, `stable-diffusion-xl`, `stable-diffusion-refiner`
- **自定义模型**: `stable-diffusion-custom`

#### 智能模型解析

- 基于优先级排序自动选择最优模型（1 = 官方，2 = 企业，3 = 社区）
- 支持多种量化格式（FP8、GGUF、NF4 等）
- SD3.5 自动检测 CLIP L/G/T5 编码器配置
- TTL 缓存系统减少 90% 的服务器请求

#### 高级特性

- **分层错误处理**: ConfigError、WorkflowError、ServicesError
- **认证管理器**: 支持 4 种认证方式，自动失败恢复
- **工作流路由**: 智能模型变体识别和工作流选择
- **组件管理**: LoRA、ControlNet 的自动兼容性检测
