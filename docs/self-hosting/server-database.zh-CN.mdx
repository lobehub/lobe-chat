...
<Callout type={'tip'}>
  由于 `NEXT_PUBLIC` 开头的环境变量是在前端代码中生效的，而因此无法通过容器运行时注入进行修改。 （参考 `next.js`的文档 [Configuring: Environment Variables | Next.js (nextjs.org)](https://nextjs.org/docs/pages/building-your-application/configuring/environment-variables) 这也是为什么我们选择再打一个 DB 版的镜像的原因。

如果需要修改 `NEXT_PUBLIC` 前缀的变量，你需要自行构建镜像，在 build 时就把自己的 `NEXT_PUBLIC` 开头的环境变量打进去。
