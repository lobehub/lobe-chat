...
<Steps>
  ...
  If you want to use Amazon Bedrock models, you can configure it using the Bearer Token authentication:

  ```fish
  $ docker run -d -p 3210:3210 \
    -e AWS_BEARER_TOKEN_BEDROCK=your_bearer_token_here \
  ...
  For more information about obtaining Bedrock API keys, see the [Bedrock Provider Guide](/docs/usage/providers/bedrock).
