...
<Steps>
  ...
  在完成数据库 DATABASE\_URL 环境变量添加后，需要添加一个 `KEY_VAULTS_SECRET` 环境变量。该变量用于加密用户存储的 apikey 等敏感信息。你可以使用 `openssl rand -base64 32` 生成一个随机的 32 位字符串作为密钥。
  ...
  该部分最后需要添加 APP\_URL 环境变量，用于指定 LobeChat 应用的 URL 地址。
</Steps>
