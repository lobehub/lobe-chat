...
为方便快速上手，这一章使用 `docker-compose/local` 目录中的 docker-compose 配置文件快速启动。启动后的 LobeChat 应用运行在 `http://localhost:3210` 端口，适用于本地设备运行。

<Callout type="warning">
  为了快速上手，此 docker-compose.yml内置掉了大量的 SECRET/Password
  的配置，仅适用于快速演示或个人本地使用。请勿直接用于线上生产环境！遇到安全问题请自己负责！
</Callout>
...
```sh
curl -fsSL https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/docker-compose.yml > .env
...
2. 创建一个 `Next.js (App Router)` 应用，添加以下配置：
   ...
   - （可选）`CORS allowed origins` 为 `http://localhost:3210`
