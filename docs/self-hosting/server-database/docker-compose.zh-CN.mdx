...
```sh
curl -fsSL https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/setup.sh > setup.sh
```
...
1. 使用 `setup.sh` 脚本启动后，Casdoor WebUI 默认端口为 `8000`，你可以通过 `http://localhost:8000` 访问，默认用户名 `admin`，密码 `123`

2. 大多数配置保持默认即可，你也可以在 `身份认证 -> 应用` 中修改默认配置

### 配置 MinIO S3

1. 使用 `setup.sh` 脚本启动后，MinIO WebUI 默认端口为 `9001`，你可以通过 `http://localhost:9001` 访问，默认用户名 `YOUR_MINIO_USER`，密码 `YOUR_MINIO_PASSWORD`
...
4. 在 Casdoor 的 `身份认证 -> 提供商` 中关联 MinIO S3 服务，以下是一个示例配置：
    ...
    ![casdoor](https://github.com/user-attachments/assets/75dc5fa7-904c-4107-9012-dcc942a9efd7)

    其中，客户端 ID、客户端密钥为上一步创建的访问密钥中的 `Access Key` 和 `Secret Key`
...
下文假设局域网内需要运行 LobeChat 服务的机器 IP 为 `your_server_ip`。
...
随后，打开 `http://your_server_ip:8000` 访问 Casdoor WebUI，在 `身份认证 -> 应用` 中添加一行
...
你也可以仿照前文一样，设置 MinIO 以支持更换头像功能，但是请注意将所有字段中的 `localhost` 替换为 `your_server_ip`。

## 发布到生产（公网域名）

<Callout type="info">
  对于拥有域名的部署，我们推荐使用 [Logto](https://logto.io/) 作为登录鉴权服务。本文剩余部分也将以其为例进行说明。
