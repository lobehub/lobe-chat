...
<Callout type="info">
当前仅支持 Unix 环境的一键启动，Windows 用户请在 [WSL](http://aka.ms/wsl) 中运行。
同时，该脚本仅支持首次部署，如果你非首次部署，请参考 [自定义部署](#自定义部署) 章节。
开始前，应该检查端口`3210`、`8000`、`9000`、`9001`是否被占用，如果被占用，请先关闭占用端口的服务。
</Callout>

新建一个空目录 `lobe-chat-db`，用于存放你的配置文件和后续的数据库文件。

```sh
mkdir lobe-chat-db && cd lobe-chat-db
```

运行下述命令，将会下载一键启动 `setup.sh` 脚本并执行：

```sh
bash <(curl -fsSL https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/setup.sh) -f -l zh_CN
```

运行后，脚本将会询问你需要的部署模式，请根据你的需求选择相应的模式，并继续阅读文档的剩余部分。

- [本地模式（默认）](#本地模式)：仅能在本地访问，不支持局域网 / 公网访问，适用于初次体验。
- [端口模式](#端口模式)：支持局域网 / 公网的 `http` 访问，适用于个人使用。
- [域名模式](#域名模式)：支持局域网 / 公网在使用反向代理下的 `http/https` 访问，适用于团队使用。

<Callout type="info">
  在脚本的选项提示 `(选项1/选项2)[选项1]` 中：`选项1 / 选项2` 代表合法的输入，方括号中的选项为默认选项，直接回车即可选择默认选项。
</Callout>
...
<Steps>
  ...
  Casdoor:
    ...
    - Username: admin
    - Password: 837e26
    - Client Secret: dce63693be24b5d9b9536126692e3ed9
  Minio:
  ...
  你可以通过 [http://localhost:3210](http://localhost:3210) 访问你的 LobeChat 服务。应用的账号密码在步骤`2`的报告中。
</Steps>
...
<Steps>
  ...
  应用的账号密码在步骤`2`的报告中。
  同时，如果你的 LobeChat 服务允许多用户访问，我们建议你参照 [登录鉴权服务配置](#登录鉴权服务配置) 章节
  配置 Casdoor 一个新的 Casdoor 组织及应用提供非管理员用户的登录服务。
</Steps>
...
<Steps>
  ...
  - `auth-ui.example.com`：`8000`
  - `s3-api.example.com`：`9000`
  - `s3-ui.example.com`：可选，`9001`
...
在开始前，你可以先下载 [Docker Compose 配置文件](https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/docker-compose.yml) 以及 [环境变量配置文件](https://raw.githubusercontent.com/lobehub/lobe-chat/HEAD/docker-compose/local/.env)。  
