...
以下是参考[自动同步更新 - LobeChat](https://lobehub.com/zh/docs/self-hosting/advanced/upstream-sync)文档更改的自动更新LobeChat版本的脚本。
请将其保存到与上文创建的环境 `lobe-chat-db` 文件夹中。
...
if docker-compose pull | grep -q "Image is up to date"; then
  ...
  echo "Current Version: $(docker inspect lobehub/lobe-chat-database:latest | grep 'org.opencontainers.image.version' | awk -F'"' '{print $4}')"
  exit 0
fi
...
echo "New Version: $(docker inspect lobehub/lobe-chat-database:latest | grep 'org.opencontainers.image.version' | awk -F'"' '{print $4}')"
...
Crontab请参考[自动同步更新 - LobeChat](https://lobehub.com/zh/docs/self-hosting/advanced/upstream-sync)文档 配置 Crontab 自动执行脚本 小节配置。
