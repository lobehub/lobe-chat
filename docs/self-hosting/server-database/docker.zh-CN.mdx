...
<Callout type="info">
  本文已经假定你了解了 LobeChat
  服务端数据库版本的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 LobeChat
  服务端数据库版本的部署原理，请先查阅 [使用服务端数据库部署](/zh/docs/self-hosting/server-database)
  。
</Callout>
...
<Callout type="info">
  pgvector 插件为 Postgres 提供了向量搜索的能力，是 RAG 中的重要构件之一。
</Callout>

<Callout type="warning">
  以上命令指令的 pg 实例并没有指定持久化存储位置，因此仅用于测试/演示，请勿直接用于生产
</Callout>
...
<Callout type="tip">
  ...
  ，我们会确保从一个空数据库到所有表正式可用的稳定性。因此我们建议你的数据库实例使用一个空表实例，进而省去手动维护表结构
  migration 的成本。
</Callout>
