...
- **广泛模型支持**：支持 127 个模型，包含 FLUX 系列（124 个）和 SD3.5 系列（3 个）
- **配置驱动架构**：4 个注册表系统提供智能模型选择和 O (1) 查找性能
...
LobeChat ComfyUI 集成采用配置驱动的架构，通过 4 个注册表系统支持 **127 个模型**，提供从官方模型到社区优化版本的全覆盖。
...
- **Priority 1**：`sd3.5_large.safetensors` - 大型基础模型
- **Priority 2**：`sd3.5_large_turbo.safetensors` - 快速生成版本
- **Priority 3**：`sd3.5_medium.safetensors` - 中等规模模型
...
<Steps>
  ...
  LobeChat 支持 127 个模型，您可以根据需求选择下载：
