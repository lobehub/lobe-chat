...
**允许的用户 ID** 是私信 **白名单** 和 **配对审批** 策略使用的已批准用户列表。**开放** 会忽略它，群聊 `@提及` 则由 **群组策略** 独立控制。`open_id` 可从事件 payload 读取，或在 Lark 开发者后台查看 **User ID**。

### 私信策略

- **开放 (Open)（默认）** — 租户内任何成员都可以私信机器人，忽略 **允许的用户 ID**。
...
| **允许的用户 ID**               | 否    | 私信白名单和配对审批策略使用的 Lark `open_id`                             |
