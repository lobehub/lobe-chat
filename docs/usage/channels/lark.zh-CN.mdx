...
> **注意：** 下文第四步中在 Lark 开放平台配置 **Event Subscription URL** 的步骤仅适用于 **Webhook 模式**。如果您使用默认的 **WebSocket** 模式，请跳过这些步骤。
...
<Steps>
  ...
  - **Webhook** — 适用于已在 Lark 开放平台配置了 Event Subscription URL 的应用，请继续下面的步骤。

  > 此时您不需要填写 **Verification Token** 或 **Encrypt Key** —— 可以在完成第四步（仅 Webhook 模式需要）配置事件订阅后再设置。

  ### 保存配置（仅 Webhook 模式）

  点击 **Save Configuration**。在 **Webhook 模式** 下，保存后将显示一个 **Event Subscription URL**，复制此 URL —— 您将在下一步中需要它。在 **WebSocket 模式** 下，机器人会自动连接，不会生成该 URL。

  ![](/blog/assets0a25d3ffb02d35f6f28cdfa9da2dccd8.webp)
</Steps>

## 第四步：在 Lark 中设置事件订阅（仅 Webhook 模式）
...
- **机器人无法连接（WebSocket 模式）：** 验证 App ID 和 App Secret 是否正确，并确认机器人功能已启用且已注册 `im.message.receive_v1` 事件。
