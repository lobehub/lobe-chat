...
私信和访客模式的 **白名单** / **配对审批** 策略共用这份已批准用户列表。两个 **开放** 策略都会忽略它，群聊 `@提及` 则由 **群组策略** 独立控制。让目标用户私信 [@userinfobot](https://t.me/userinfobot) 拿到自己的数字 ID，或直接从机器人收到的 update 里读 `from.id`。

### 私信策略

- **开放 (Open)（默认）** — Telegram 上任何用户都可以私信机器人，忽略 **允许的用户 ID**。
...
- **开放 (Open)（默认）** — 接受任何人的 Guest 召唤，忽略 **允许的用户 ID**。
...
| **允许的用户 ID**     | 否    | 私信和访客模式的白名单 / 配对审批策略共用的 Telegram 数字用户 ID                    |
