...
## Quick Create a Feishu Agent App (Recommended)

For a new bot, use **创建飞书智能体应用** (Create Feishu Agent App) on the Feishu Open Platform. It presets permissions and events for an agent app. Connect it to your LobeHub agent by entering the resulting credentials in LobeHub. This creates the Feishu app; you still choose the agent it connects to in LobeHub.

<Steps>
  ### Open Quick Creation

  Sign in to the [Feishu Open Platform](https://open.feishu.cn/app). At the top of the app list, find **创建飞书智能体应用** and click **立即创建** (Create Now).

  ![Create Feishu Agent App banner at the top of the developer portal](https://app.lobehub.com/f/file_uGpHMerJI5MX)

  ### Choose an Avatar and Name

  Select an avatar, enter an app name such as "LobeHub Assistant", and click **立即创建** (Create Now). Wait for the automatic configuration to finish.

  ![Choose an avatar and name for the Feishu agent app, then click Create Now](https://app.lobehub.com/f/file_33DKY95u6mRR)

  ### Copy App Credentials

  On the **创建成功** (Created Successfully) page, copy **App ID** and **App Secret**. Keep the App Secret private: do not send it to a group chat or include it in a public screenshot.

  ![Created Successfully page showing App ID with App Secret hidden](https://app.lobehub.com/f/file_kRwHcq8krjkA)

  ### Connect to LobeHub

  Open the agent you want to connect in LobeHub, then go to **Settings** → **Channels** → **飞书** (Feishu). Enter the **App ID** and **App Secret** you copied. Keep the default **WebSocket** connection mode under **Advanced Settings** and click **Save Configuration**.
  ...
  Follow the Feishu page prompts to confirm the app is published and available. Complete administrator approval first if your organization requires it. Click **打开应用** (Open App) and send the bot a message to confirm that your LobeHub agent replies.

  If it does not reply, check that the bot capability is enabled, the subscription method is **使用长连接接收事件** (Use long connection to receive events), `im.message.receive_v1` is included, and the channel configuration has been saved in LobeHub. See Troubleshooting below for further checks.
</Steps>

The screenshots show the Chinese Feishu interface with an example app named "Lobehub 周报小助手" (LobeHub Weekly Report Assistant). Use your own app name.
...
<Steps>
  ...
  - **WebSocket** (default) — Recommended for new bots. After saving, continue to Step 4 and enable long-connection event delivery on the Feishu Open Platform.
...
**Allowed User IDs** is the approved-user list used by DM **Allowlist** and **Pairing** policies. **Open** ignores it, and group `@mentions` are controlled independently by **Group Policy**. Read the `open_id` from the event payload, or copy the **User ID** displayed in the Feishu Developer Portal.

### DM Policy

- **Open (default)** — Any tenant member can DM the bot. **Allowed User IDs** is ignored.
...
| **Allowed User IDs**       | No       | Feishu `open_id` values used by DM Allowlist and Pairing policies                                    |
...
- **Bot not connecting (WebSocket mode):** Verify the App ID and App Secret are correct and that the LobeHub service can reach the Feishu Open Platform.
- **Connected but not receiving messages (WebSocket mode):** Confirm the Feishu subscription method is **Use long connection to receive events**, `im.message.receive_v1` is added, and an app version containing those changes has been published.
