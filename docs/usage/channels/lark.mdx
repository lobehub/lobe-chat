...
**Allowed User IDs** is the approved-user list used by DM **Allowlist** and **Pairing** policies. **Open** ignores it, and group `@mentions` are controlled independently by **Group Policy**. Read the `open_id` from the event payload, or copy the **User ID** displayed in the Lark Developer Portal.
...
| **Allowed User IDs**       | No       | Lark `open_id` values used by DM Allowlist and Pairing policies                                     |
