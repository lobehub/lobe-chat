...
A populated **Allowed User IDs** field acts as a global gate — DMs, group `@mentions`, and Guest Mode summons are restricted to listed Telegram numeric user IDs. Empty means "no user-level filter". Grab a user's numeric ID via [@userinfobot](https://t.me/userinfobot), or read the `from.id` field from the bot's incoming update.
...
- **Open (default)** — Accept Guest Mode summons from anyone. A populated **Allowed User IDs** list still acts as a global user gate.
...
| **Allowed User IDs**     | No       | Comma- or whitespace-separated Telegram numeric user IDs. Global gate — applies to DMs, group @mentions, and Guest Mode |
