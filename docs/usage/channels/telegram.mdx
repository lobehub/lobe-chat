...
Guest summons are not gated by **Group Policy** — they are already opt-in per message. **Allowed User IDs** still apply. Keyword watch and always-on group traffic still require adding the bot as a member.
