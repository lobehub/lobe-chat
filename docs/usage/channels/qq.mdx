...
**Allowed User IDs** is the approved-user list used by DM **Allowlist** and **Pairing** policies. **Open** ignores it, and group `@mentions` are controlled independently by **Group Policy**. Read the `user_openid` from the OpenAPI event payload; the visible QQ number is not always the same as the platform ID.

### DM Policy

- **Open (default)** — Any QQ user who shares context with the bot can DM it. **Allowed User IDs** is ignored.
...
| **Allowed User IDs**    | No       | QQ `user_openid` values used by DM Allowlist and Pairing policies                       |
