...
LobeChat 提供了内置的客户端数据库体验。但某些重要功能仅在服务端开发中可用。

为了使用服务端数据库功能，需要参考 [部署服务端数据库](https://lobehub.com/docs/self-hosting/server-database) 的说明来配置所有前置条件。本文档提供了一个更简化的配置方法，能够在本地开发时快速启动简化的服务端环境。
