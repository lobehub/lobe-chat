...
有关目录架构的详细介绍，详见： [文件夹目录架构](folder-structure.zh-CN.mdx)
...
现在，你应该可以在浏览器中看到 LobeChat 的欢迎页面。详细的环境配置指南，请参考 [开发环境设置指南](setup-development.zh-CN.mdx)。
...
详细的代码风格和贡献指南，请参考 [代码风格与贡献指南](contributing-guidelines.zh-CN.mdx)。
...
如果要添加新语种，需遵循特定步骤，详见 [新语种添加指南](./internationalization/add-new-locale.zh-CN.mdx)。 我们鼓励你参与我们的国际化努力，共同为全球用户提供更好的服务。

详细的国际化实现指南，请参考 [国际化实现指南](./internationalization/internationalization-implementation.zh-CN.mdx)。

## 附录：资源与参考

为了支持开发者更好地理解和使用 LobeChat 的技术栈，我们提供了一份详尽的资源与参考列表 —— [LobeChat 资源与参考](./basic/resources.zh-CN.mdx) - 访问我们维护的资源列表，包括教程、文章和其他有用的链接。
