...
| 环境变量               | 类型 | 说明                                                                                        | 示例                            |
| ------------------ | -- | ----------------------------------------------------------------------------------------- | ----------------------------- |
| `OPENAI_API_KEY`   | 必选 | 这是你在 OpenAI 账户页面申请的 API 密钥                                                                | `sk-xxx...xxx`                |
| `OPENAI_PROXY_URL` | 可选 | 如果你手动配置了 OpenAI 接口代理<br/>可以使用此配置项来覆盖默认的 OpenAI API 请求基础 URL<br/>默认值: `https://api.openai.com` | `https://api.chatanywhere.cn` |
